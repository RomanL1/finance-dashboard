...
- Sign-up requires a verified email. In e2e, create users with `signUpVerified` (`test/support/auth.ts`); mails land as JSON in `MAIL_OUTBOX_DIR` (`test/support/mail.ts`).
